...
indels_by_chr = defaultdict(lambda : defaultdict(str))
...
for s in ('A', 'ARK', 'BALBc', 'C3HHe', 'DBA2'):
    with open('data/' + s + '_insertions.bed', 'r') as f:
        ...
        while line != '':
            cols = line.split('\t')
            indels_by_chr[cols[0]][(int(cols[1]), int(cols[2]))] += s + '_'
...
with open('data/ancestor_indels.bed', 'w') as f:
    for curr_chr in ('chr1', 'chr2', 'chr3', 'chr4', 'chr5', 'chr6', 'chr7', 'chr8', 'chr9', 'chr10', 'chr11', 'chr12',
        ...
        sorted_dict = sorted(indels_by_chr[curr_chr].iteritems(), key=itemgetter(0))
