...
def write_statistics(filename_in, ancestors, SNPs, starting_params, run_count):
    len_before = len(SNPs)
    len_after = 0
    ...
    line = '%i\t%i\t%.3f\t%.5f\t%.5f\t%.2f\t%.2f\t%.1f\t%s\t%s\t%s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%i\n' % \
    ...
    if not isfile(filename_out):
        ...
        Use_recomb_rate\t%_A\t%_ARK\t%_BALBc\t%_C3HHe\t%_C57BL6N\t%_DBA2\t%_Unknown\tRun_count\n' + line
